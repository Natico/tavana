<?php
/**
 * Standard product frontend template.
 *
 * @package ArtTheme
 */
...
$product_code = get_post_meta($post_id, '_art_product_code', true);
$design_year = get_post_meta($post_id, '_art_product_design_year', true);
$description = get_the_content(null, false, $post_id);
$categories = get_the_terms($post_id, ProductCategoryTaxonomy::KEY);
?>

<article id="post-<?php the_ID(); ?>" <?php post_class('product-single product-template product-template--standard'); ?>>
	...
	<?php if ($is_visible(ProductTemplateRegistry::FIELD_PRODUCT_CODE) && is_string($product_code) && '' !== $product_code) : ?>
		<section class="product-single__section">
			<h2><?php esc_html_e('Product Code', 'art-theme'); ?></h2>
			<p><?php echo esc_html($product_code); ?></p>
		</section>
	<?php endif; ?>

	<?php if ($is_visible(ProductTemplateRegistry::FIELD_DESIGN_YEAR) && '' !== (string) $design_year) : ?>
		<section class="product-single__section">
			<h2><?php esc_html_e('Design Year', 'art-theme'); ?></h2>
			<p><?php echo esc_html((string) $design_year); ?></p>
		</section>
	<?php endif; ?>
